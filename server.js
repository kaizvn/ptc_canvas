...
GLOBAL.__clientDir = __dirname + '/public';
...
app.post('/uploads', (req, res) => {
	upload(req, res, err => {
		if (err) {
			...
			let newUrl = req.protocol + '://' + req.get('host') + '/images/' + req.file.filename
			io.emit('newImage', newUrl);
			res.status(200).json({
				file: newUrl
			})
		}
...
// general route
app.get('/', (req, res) => {
	res.sendFile(__clientDir + '/index.html');
});
...
app.get('/images', (req, res) => {
	...
	let files = fs.readdirSync(__clientDir + '/images/');
	files = files
